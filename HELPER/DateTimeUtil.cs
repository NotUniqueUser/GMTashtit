...
namespace HELPER
{
    public class DateTimeUtil
    {
        ...
        {
            var calendar = Calendar.GetInstance(TimeZone.GetTimeZone("Asia/Jerusalem"));
        ...
        public static bool IsValidTime(string time)
        {
            ...
            if (IsValidDate(time))
                t = time.Substring(11); // יש לחלץ את השעה מהתאריך
            else
                t = time; //          התקבלה שעה בלבד  
        ...
        {
            var isok = startDate.Date <= dateToCheck.Date && dateToCheck.Date <= endDate.Date;
